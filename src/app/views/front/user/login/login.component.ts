    ...
    if (this.loginform.valid) {
      this.service.GetUserbyCode(this.loginform.value.id).subscribe((item) => {
        ...
        if (this.result && this.result.password === this.loginform.value.password) {
          ...
          sessionStorage.setItem('cin', this.result.cin);

          if (this.result.role === 'admin') {
            console.log(this.result.role);
            // i want here to add logic like variable hidden to make it roleA='admin' i want to do that for securite becouse can utilisatuer change role in sessionStorage so i want to test if (role == roleA after starting login) 
            const encodedRole = btoa('admin');
            sessionStorage.setItem('token', encodedRole);
            this.router.navigate(['/admin/foyer/Dashboard']);
          } else if (this.result.role === 'user') {
            const encodedRole = btoa('user');
            sessionStorage.setItem('token', encodedRole);
